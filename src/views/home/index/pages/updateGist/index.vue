    ...
    <div id="app">
        <div class="row1">
            ...
            <div class="btns">
                ...
                <div class="updateBtn" @click="updateBtn">
                    发布
                </div>
            </div>
...
<script>
    ...
    import {ADD_POST_LABEL, GET_LABEL_ONE, GET_LABELS, UPDATE_LABEL} from "../../../../../assets/apis/label";
    ...
    export default {
        ...
        mounted() {
            ...
            /*get data*/
            var self = this;
            this.$data.group.id = getUrlParams()['groupId']
            this.get_labels()
            get_gist_one(getUrlParams()['gistId']).then(res => {
                // content: "1"
                // group_id: 132
                // id: 1
                // title: "1"
                self.$data.input_value = res.content
                self.$data.title_value = res.title
                self.$data.label_value = res.label_id + ""
                setTimeout(() => {
                    self.$refs["show_text"].innerHTML = mkToHmtl(self.$refs['input_value'].value)
                }, 300)
        ...
        methods: {
            ...
            async updateBtn() {
                ...
                MyAlert.loading()
                put_gists(gist).then(res => {
                    MyAlert.clear()
                    MyAlert.success("update gist is success")
